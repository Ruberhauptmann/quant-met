...
class ThreeBand(BaseHamiltonian):
    """Hamiltonian for Graphene."""

    def __init__(
        self,
        parameters: ThreeBandParameters,
        *args: tuple[Any, ...],
        **kwargs: tuple[dict[str, Any], ...],
        ...
        self._hubbard_int_orbital_basis = np.array(
            [self.hubbard_int, self.hubbard_int, self.hubbard_int]
        )
    ...
    def from_file(cls, filename: pathlib.Path) -> "BaseHamiltonian":  # noqa: D102
        ...
        parameters = ThreeBandParameters.model_validate(config_dict)
